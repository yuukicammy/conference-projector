...
class Layout:
    ...
    def paper_title_block(self, df: pd.core.frame.DataFrame, index:int, rank:int) -> dbc.Row:
        title_text = f'{str(rank)}.  {df.at[index, "title"]}' if 0 < rank else f'{df.at[index, "title"]}'
        return dbc.Row(
            [
                dbc.Col(
                    dcc.Link(
                        ...
                        id="paper-title",
                        target="_blank",
                        style={"fontSize": 16, "margin-bottom": "5px"}, #"color": "#316745",
                    ),
                    width="90%",
                ),
            ]
    ...
    def paper_info_base(self, df: pd.core.frame.DataFrame, index:int) -> dbc.Row:
        return dbc.Row(
            [
                dbc.Col(
                    dcc.Markdown(
    ...
    def paper_block(self, df: pd.core.frame.DataFrame, index:int, rank:int, options: List[str]) -> dbc.Row:
        title_block = self.paper_title_block(df=df, index=index, rank=rank)
        info_base = self.paper_info_base(df=df, index=index)
        ...
        for key in self.container_data.info_order:
            if key in options:
                ...
                if key in self.container_data.properties.keys():
                    description += f'#### {self.container_data.properties[key]["description"]}\n'
                    description += f"{df.at[index, key]}\n"

        return dbc.Row([title_block, info_base, dcc.Markdown(description)])
    
    def description_block(
        self, df: pd.core.frame.DataFrame, indices: List[int], options: List[str], start_rank:int, im_width="100%"
    ) -> dbc.Row:
        content = []
        if indices is None or len(indices) == 0:
            ...
            for i, index in enumerate(indices):
                ...
                if index < self.container_data.num_data:
                    info = self.paper_block(df=df, index=index, rank=rank, options=options)
                    if "image" in options:
    ...
    def selected_paper_block(self, df: pd.core.frame.DataFrame, index: int) -> dbc.Col:
        content = self.description_block(
            df=df,
            indices=[index],
            options=list( # remove description_en because it is almost same as the abstract. 
                filter(lambda x: x != "description_en", self.container_data.info_opt_vals) 
            ),
            start_rank=0,
            im_width="100%",
        )
    ...
    def recommendation_block(self, options: List[str], all_options: List[Dict[str, str]]) -> dbc.Col:
        return dbc.Col(
            dbc.Row(
                [
                    self.top_margin,
    ...
    def navbar(self) -> dbc.NavbarSimple:
        ...
        brand_href="https://yuukicammy--paper-viz-webapp-wrapper.modal.run",
        color="dark",
        dark=True,
